...
static int pnx8550_timers_read(char* page, char** start, off_t offset,
		int count, int* eof, void* data)
{
	...
	if (offset==0) {
		...
		len += sprintf(&page[len], "    1: %10u, %10u,  %1i, %s\n",
				...
				(configPR >> 6) & 1, ((configPR >> 3) & 1) ? "off" : "on");
		len += sprintf(&page[len], "    2: %10u, %10u,  %1i, %s\n",
				read_c0_count2(), read_c0_compare2(),
				(configPR >> 7) & 1, ((configPR >> 4) & 1) ? "off" : "on");
		len += sprintf(&page[len], "    3: %10u, %10u,  %1i, %s\n",
				read_c0_count3(), read_c0_compare3(),
				(configPR >> 8) & 1, ((configPR >> 5) & 1) ? "off" : "on");
	}

	return len;
}

static int pnx8550_registers_read(char* page, char** start, off_t offset,
		int count, int* eof, void* data)
{
	int len = 0;
...
static int pnx8550_clocks_read(char* page, char** start, off_t offset,
		int count, int* eof, void* data)
{
	int len = 0;
...
static int pnx8550_proc_init(void)
{
	...
	if (!pnx8550_dir) {
		printk(KERN_ERR "cannot create /proc/pnx8550\n");
		return -1;
	}

	// Create /proc/pnx8550/timers
	pnx8550_timers = create_proc_read_entry("timers", 0, pnx8550_dir,
			pnx8550_timers_read, NULL);
	if (!pnx8550_timers)
		printk(KERN_ERR "cannot create /proc/pnx8550/timers\n");

	// Create /proc/pnx8550/registers
	pnx8550_registers = create_proc_read_entry("registers", 0, pnx8550_dir,
			pnx8550_registers_read, NULL);
	if (!pnx8550_registers)
		printk(KERN_ERR "cannot create /proc/pnx8550/registers\n");

	// Create /proc/pnx8550/clocks
	pnx8550_clocks = create_proc_read_entry("clocks", 0, pnx8550_dir,
			pnx8550_clocks_read, NULL);
	if (!pnx8550_clocks)
		printk(KERN_ERR "cannot create /proc/pnx8550/clocks\n");
