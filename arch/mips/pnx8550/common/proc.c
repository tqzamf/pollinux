...
static int pnx8550_registers_read(char* page, char** start, off_t offset, int count, int* eof, void* data)
{
        ...
        if (offset==0) {
                len += sprintf(&page[len], "prid:      %#10.8x\n", read_c0_prid());
                len += sprintf(&page[len], "config:    %#10.8x\n", read_c0_config());
                ...
                len += sprintf(&page[len], "debug:     %#10.8x\n", read_c0_debug());
                len += sprintf(&page[len], "wired:     %#10.8x\n", read_c0_wired());
                len += sprintf(&page[len], "pagemask:  %#10.8x\n", read_c0_pagemask());
                len += sprintf(&page[len], "pagegrain: %#10.8x\n", read_c0_pagegrain());
                ...
                len += sprintf(&page[len], "cmem_0:    %#10.8x\n", read_c0_diag4());
                len += sprintf(&page[len], "cmem_1:    %#10.8x\n", read_c0_diag5());
                len += sprintf(&page[len], "cmem_2:    %#10.8x\n", read_c0_diag6());
                len += sprintf(&page[len], "cmem_3:    %#10.8x\n", read_c0_diag7());
                len += sprintf(&page[len], "hwrena:    %#10.8x\n", read_c0_hwrena());
                len += sprintf(&page[len], "user:      %#10.8lx\n", read_c0_userlocal());
        }
